...
/obj/machinery/recharger/defibcharger/wallcharger/togglePanelOpen(var/obj/toggleitem, var/mob/user)
	if(charging)
		user << "<span class='indigo'>Not while [src] is charging!</span>"
		return
	return(..())
...
obj/machinery/recharger/defibcharger/wallcharger/attackby(obj/item/weapon/G as obj, mob/user as mob)
	...
	if(istype(G, /obj/item/weapon/melee/defibrillator))
		...
		if(D.ready)
			user << "<span class='indigo'>[D] won't fit. Try putting the paddles back on!</span>"
			return
		if(charging)
			user << "<span class='indigo'>Remove [D] first!</span>"
			return
		// Checks to make sure he's not in space doing it, and that the area got proper power.
		var/area/a = get_area(src)
		if(!isarea(a))
			user << "<span class='indigo'>[src] blinks red as you try to insert [D]!</span>"
			return
		if(a.power_equip == 0)
			user << "<span class='indigo'>[src] blinks red as you try to insert [D]!</span>"
			return
