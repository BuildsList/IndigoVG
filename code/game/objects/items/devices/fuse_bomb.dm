...
/obj/item/device/fuse_bomb/attack_self(mob/user as mob)
	if(!fuse_lit)
		...
		fuse_lit = 0
		update_icon()
		user << "<span class='indigo'>You extinguish the fuse with [seconds_left] seconds left!</span>"
	return
...
/obj/item/device/fuse_bomb/proc/lit(mob/user as mob, var/obj/O=null)
	fuse_lit = 1
	user << "<span class='indigo'>You lit the fuse[O ? " with [O]":""]! [seconds_left] seconds till detonation!</span>"
	admin_warn(user)
