	...
	if(stat == 2)
		...
		for(var/mob/M in viewers(src, null))
			if((M.client && !( M.blinded )))
				M.show_message("<span class='indigo'> [src] lets out a contented sigh as their form unwinds.</span>")
				ghostize()
		del src
		return
...
/mob/living/simple_animal/shade/attackby(var/obj/item/O as obj, var/mob/user as mob)  //Marker -Agouri
	...
	if(istype(O, /obj/item/device/soulstone))
		...
		if(O.force)
			...
			for(var/mob/M in viewers(src, null))
				if ((M.client && !( M.blinded )))
					M.show_message("<span class='indigo'> \b [src] has been attacked with [O] by [user].</span>")
		else
			usr << "<span class='indigo'> This weapon is ineffective, it does no damage.</span>"
			for(var/mob/M in viewers(src, null))
				if ((M.client && !( M.blinded )))
					M.show_message("<span class='indigo'> [user] gently taps [src] with [O].</span>")
	return
