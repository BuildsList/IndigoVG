/mob/living/silicon/robot/mommi/examine(mob/user)
	...
	msg += {"<p>It's like a crab, but it has a utility tool on one arm and a crude metal claw on the other.  That, and you doubt it'd survive in an ocean for very long.</p><span class='indigo'>"}
	// END AUTOFIX
	if (src.getBruteLoss())
	...
	if(opened)
		msg += "<span class='indigo'>Its cover is open and the power cell is [cell ? "installed" : "missing"].</span>\n"
	else
	...
	switch(src.stat)
		...
		if(UNCONSCIOUS)		msg += "<span class='indigo'>It doesn't seem to be responding.</span>\n"
