...
/mob/living/carbon/human
		...
		if(disabilities & EPILEPSY)
			if((prob(1)) && (paralysis < 1))
				visible_message( \
					...
					"<span class='indigo'>You have a seizure!</SPAN>")
	...
	proc/breathe()
		...
		if(L)
			...
			src << "<span class='indigo'>You have no lungs which to breathe with, panic and tell a coder.</span>"
			...
			if (!istype(wear_suit,/obj/item/clothing/suit/space/plasmaman) || !istype(head,/obj/item/clothing/head/helmet/space/plasmaman))
				...
				if(environment)
					if((environment.oxygen / environment.total_moles()) >= OXYCONCEN_PLASMEN_IGNITION) //how's the concentration doing?
						if(!on_fire)
							src << "<span class='indigo'>Your body reacts with the atmosphere and bursts into flame!</span>"
	...
	if(head)
		if(istype(head, /obj/item/weapon/reagent_containers/glass/bucket))
			...
			if(B.reagents.total_volume)
				...
				visible_message("<span class='indigo'>The bucket's content spills on [src]</span>")
