...
/datum/surgery_step/internal/replace_organ
	...
	can_use(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
		...
		if(!target.species)
			user << "<span class='indigo'>You have no idea what species this person is. Report this on the bug tracker.</span>"
			return 0
		...
		if(target.species.has_organ[O.organ_tag])
			...
			if(!O.health)
				user << "<span class='indigo'>\The [O.organ_tag] [o_is] in no state to be transplanted.</span>"
				return 0

			if(!target.internal_organs_by_name[O.organ_tag])
				organ_missing = 1
			else
				user << "<span class='indigo'>\The [target] already has [o_a][O.organ_tag].</span>"
				return 0

			if(O.organ_data && affected.name == O.organ_data.parent_organ)
				organ_compatible = 1
			else
				user << "<span class='indigo'>\The [O.organ_tag] [o_do] normally go in \the [affected.display_name].</span>"
				return 0
		else
			user << "<span class='indigo'>\A [target.species.name] doesn't normally have [o_a][O.organ_tag].</span>"
			return 0
